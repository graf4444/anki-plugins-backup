...
class ForvoParser:
    def lookup(self, language, word, data):
        ...
        if '-' in language or '_' in language:
            div = doc.find('li', class_=language.replace('-', '_'))
        else:
            div = doc.find('div', id='language-container-{0}'.format(language))
        play = div.find('div', class_='play', title='Listen {0} pronunciation'.format(word)) if div else None
        return self._parse_tag(play) if play else None
...
class Forvo(ParsedSource):
    ...
    def get_initial_url(self, word):
        return 'https://forvo.com/word/{0}/#{1}'.format(word, "ru")
